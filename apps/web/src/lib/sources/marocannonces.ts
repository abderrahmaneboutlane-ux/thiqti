...
import { displayPhone, telHref, whatsappHref } from "./contact";
import { extractImagesFromSection } from "./extractImages";

const BASE_URLS = [
  "https://www.marocannonces.com/cat/voitures",
  "https://www.marocannonces.com/cat/voitures-occasion",
];
...
export class MarocAnnoncesCollector implements SourceCollector {
  name = "MarocAnnonces";
  ...
  async fetch(): Promise<UnifiedCar[]> {
    ...
    for (const url of BASE_URLS) {
      try {
        ...
        console.log(`[MarocAnnonces] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[MarocAnnonces] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    const sections = html.split(/(?=<div[^>]*class="[^"]*(?:annonce|listing|card|item|product)[^"]*")/i);

    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="([^"]*(?:marocannonces\.com)?[^"]*(?:\.htm|\.html)[^"]*)"/i);
        ...
        const fullUrl = linkMatch
          ? (linkMatch[1].startsWith("http") ? linkMatch[1] : `https://www.marocannonces.com${linkMatch[1]}`)
          : baseUrl;
        ...
        const verified = /v[ée]rifi[ée]/i.test(section);

        const contact: CarContact = { url: fullUrl, name: "Vendeur MarocAnnonces" };
        ...
        reputation.label = "Annonce MarocAnnonces.com";

        cars.push({
          id: generateId("marocannonces", make, model, year, km, price),
          title: rawTitle,
