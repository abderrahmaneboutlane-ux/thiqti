...
const KIFAL_URLS = [
  "https://www.kifal.ma/voitures-occasions",
  "https://www.kifal.ma/voitures-neuves",
];
...
export class KifalCollector implements SourceCollector {
  name = "Kifal";

  async fetch(): Promise<UnifiedCar[]> {
    const allCars: UnifiedCar[] = [];

    for (const url of KIFAL_URLS) {
      try {
        ...
        console.log(`[Kifal] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[Kifal] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    const sections = html.split(/(?=<div[^>]*class="[^"]*(?:annonce|listing|card|vehicle|car-card|product-card)[^"]*")/i);

    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="([^"]*(?:kifal\.ma)?[^"]*\.htm[^"]*)"/i);
        ...
        const fullUrl = linkMatch
          ? (linkMatch[1].startsWith("http") ? linkMatch[1] : `https://www.kifal.ma${linkMatch[1]}`)
          : baseUrl;
        ...
        reputation.label = verified ? "Annonce vérifiée" : "Annonce Kifal.ma";

        cars.push({
          id: generateId("kifal", make, model, year, km, price),
          title: rawTitle,
