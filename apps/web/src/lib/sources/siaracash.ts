...
const BASE_URLS = [
  "https://www.siaracash.com/voitures",
  "https://www.siaracash.com/voitures-occasion",
];
...
export class SiaraCashCollector implements SourceCollector {
  name = "SiaraCash";
  ...
  async fetch(): Promise<UnifiedCar[]> {
    ...
    for (const url of BASE_URLS) {
      try {
        ...
        console.log(`[SiaraCash] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[SiaraCash] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    const sections = html.split(/(?=<div[^>]*class="[^"]*(?:annonce|listing|card|item|product|car-card)[^"]*")/i);

    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="([^"]*(?:siaracash\.com)?[^"]*(?:\.htm|\.html)[^"]*)"/i);
        ...
        const fullUrl = linkMatch
          ? (linkMatch[1].startsWith("http") ? linkMatch[1] : `https://www.siaracash.com${linkMatch[1]}`)
          : baseUrl;
        ...
        const contact: CarContact = { url: fullUrl, name: "Vendeur SiaraCash" };
        ...
        reputation.label = "Annonce SiaraCash.com";

        cars.push({
          id: generateId("siaracash", make, model, year, km, price),
          title: rawTitle,
