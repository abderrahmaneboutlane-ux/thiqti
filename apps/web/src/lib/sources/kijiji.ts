...
const BASE_URLS = [
  "https://www.kijiji.ma/voitures/",
  "https://www.kijiji.ma/voitures/occasion/",
];
...
export class KijijiCollector implements SourceCollector {
  name = "Kijiji";
  ...
  async fetch(): Promise<UnifiedCar[]> {
    ...
    for (const url of BASE_URLS) {
      try {
        ...
        console.log(`[Kijiji] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[Kijiji] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="([^"]*(?:kijiji\.ma)?[^"]*(?:\.htm|\.html)[^"]*)"/i);
        ...
        const fullUrl = linkMatch
          ? (linkMatch[1].startsWith("http") ? linkMatch[1] : `https://www.kijiji.ma${linkMatch[1]}`)
          : baseUrl;
        ...
        const contact: CarContact = { url: fullUrl, name: "Vendeur Kijiji" };
        ...
        reputation.label = "Annonce Kijiji.ma";

        cars.push({
          id: generateId("kijiji", make, model, year, km, price),
          title: rawTitle,
