...
const WANDALOO_URLS = [
  "https://www.wandaloo.com/voitures/",
  "https://www.wandaloo.com/voitures/occasion/",
  "https://www.wandaloo.com/voitures/neuve/",
];

interface WandalooListing {
  title: string;
  price: number;
  year: number;
  km: number;
...
export class WandalooCollector implements SourceCollector {
  name = "Wandaloo";

  async fetch(): Promise<UnifiedCar[]> {
    const allCars: UnifiedCar[] = [];

    for (const url of WANDALOO_URLS) {
      try {
        ...
        console.log(`[Wandaloo] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[Wandaloo] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    const sections = html.split(/(?=<div[^>]*class="[^"]*(?:annonce|listing|card|vehicle|car-item)[^"]*")/i);

    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="(https?:\/\/(?:www\.)?wandaloo\.com[^"]*\.htm[^"]*)"/i);
        ...
        const fullUrl = linkMatch ? linkMatch[1] : baseUrl;
        ...
        reputation.label = verified ? "Annonce vérifiée" : "Annonce Wandaloo.com";

        cars.push({
          id: generateId("wandaloo", make, model, year, km, price),
          title: rawTitle,
          ...
          photos: allImages,
          inventoryType: baseUrl.includes("neuve") ? "new" as const : "used" as const,
          safety: null,
          contact,
          reputation,
        });
