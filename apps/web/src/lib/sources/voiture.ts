...
const BASE_URLS = [
  "https://www.voiture.ma/annonces/voitures",
  "https://www.voiture.ma/annonces/voitures-occasion",
];
...
export class VoitureCollector implements SourceCollector {
  name = "Voiture.ma";
  ...
  async fetch(): Promise<UnifiedCar[]> {
    ...
    for (const url of BASE_URLS) {
      try {
        ...
        console.log(`[Voiture.ma] ${url}: ${listings.length} listings`);
      } catch (err) {
        console.error(`[Voiture.ma] Failed: ${url}`, err);
      }
    }
  ...
  private parseHTML(html: string, baseUrl: string): UnifiedCar[] {
    ...
    const sections = html.split(/(?=<div[^>]*class="[^"]*(?:annonce|listing|card|item|product|car-card|vehicle)[^"]*")/i);

    for (const section of sections) {
      try {
        const linkMatch = section.match(/href="([^"]*(?:voiture\.ma)?[^"]*(?:\.htm|\.html)[^"]*)"/i);
        ...
        const fullUrl = linkMatch
          ? (linkMatch[1].startsWith("http") ? linkMatch[1] : `https://www.voiture.ma${linkMatch[1]}`)
          : baseUrl;
        ...
        const contact: CarContact = { url: fullUrl, name: "Vendeur Voiture.ma" };
        ...
        reputation.label = "Annonce Voiture.ma";

        cars.push({
          id: generateId("voiturema", make, model, year, km, price),
          title: rawTitle,
